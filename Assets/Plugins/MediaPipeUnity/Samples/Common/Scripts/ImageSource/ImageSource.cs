  ...
  public abstract class ImageSource : MonoBehaviour
  {
    ...
    public struct ResolutionStruct
    {
      ...
      public ResolutionStruct(Resolution resolution)
      {
        ...
        // Use the newer refreshRateRatio API to compute a floating-point frame rate.
        var rr = resolution.refreshRateRatio;
        frameRate = rr.denominator != 0 ? (double)rr.numerator / rr.denominator : 0.0;
      }

      public Resolution ToResolution()
      {
        // Some Unity versions don't expose a RefreshRate constructor. Use the integer
        // `refreshRate` field on Resolution and suppress the obsolete-warning here.
      #pragma warning disable CS0618
        return new Resolution() { width = width, height = height, refreshRate = (int)Math.Round(frameRate) };
